using MPIDenseLUs
using MPIDenseLUs: gather_factors!
using Combinatorics
using LinearAlgebra
using Primes

function mpi_dense_lu_test(n_shared, distributed_block_rows)
    ...
    @testset "mpi_dense_lu n_shared=$n_shared, distributed_block_rows=$distributed_block_rows" begin
        @testset "m=$m, tile_size=$tile_size" for m ∈ (32, 33, 100, 128, 295, 317, 460, 532, 604, 739, 827, 964, 1009, 1024),
            ...
            if distributed_rank == 0 && shared_rank == 0
                println("mpi_dense_lu n_shared=$n_shared, distributed_block_rows=$distributed_block_rows, m=$m, tile_size=$tile_size")
            end
            ...
            Alu = mpi_dense_lu(Afactors, tile_size, comm, shared_comm, distributed_comm,
...
function mpi_dense_lu_test_matrix_rhs(n_shared, distributed_block_rows)
    ...
    @testset "mpi_dense_lu matrix rhs, n_shared=$n_shared, distributed_block_rows=$distributed_block_rows" begin
        @testset "m=$m, tile_size=$tile_size" for m ∈ (32, 33, 100, 128, 295, 317, 460, 532, 604, 739, 827, 964, 1009, 1024),
                                                  tile_size ∈ (1, 2, 3, 25, 32, 90, 128)
            if tile_size > m + 5
                # If tile_size is bigger than m, the actual value of tile_size does not
            ...
            if distributed_rank == 0 && shared_rank == 0
                println("mpi_dense_lu matrix rhs, n_shared=$n_shared, distributed_block_rows=$distributed_block_rows, m=$m, tile_size=$tile_size")
            end

            A = allocate_array_float(m, m)
            b = allocate_array_float(m, ncols)
            x = allocate_array_float(m, ncols)
            buffer1 = allocate_array_float(m, ncols)
            buffer2 = allocate_array_float(m, ncols)
            buffer3 = allocate_array_float(ncols, m)
            ...
            function test_once()
                ldiv!(x, Alu, b, buffer1, buffer2, buffer3)
                # LU factorise using the row permutation calculated for Alu, to compare
                # the factors.
                check_factors_lu = lu(A[Alu.row_permutation,:], NoPivot())
                if distributed_nproc > 1
            ...
            @testset "change A" begin
                ...
                # Synchronization is required here when using only shared-memory MPI
                # because the short-cut branch in lu!() that factorizes using
                # MPISharedMemLUs directly does not synchronize after calculating the
                # row_permutation entries.
                MPI.Barrier(shared_comm)
...
function mpi_dense_lu_test_matrix_rhs_no_distributed(n_shared)
    ...
    @testset "mpi_dense_lu matrix rhs, no distributed, n_shared=$n_shared" begin
        @testset "m=$m, tile_size=$tile_size" for m ∈ (32, 33, 100, 128, 295, 317, 460, 532, 604, 739, 827, 964, 1009, 1024),
                                                  tile_size ∈ (1, 2, 3, 25, 32, 90, 128)
            if tile_size > m + 5
                # If tile_size is bigger than m, the actual value of tile_size does not
            ...
            if distributed_rank == 0 && shared_rank == 0
                println("mpi_dense_lu matrix rhs, no distributed, n_shared=$n_shared, m=$m, tile_size=$tile_size")
            end
...
function mpi_dense_lu_tests()
    ...
    @testset "mpi_dense_lu_tests" for n_shared ∈ [prod(x) for x ∈ unique(combinations(factor(Vector, nproc)))], distributed_block_rows ∈ (nothing, 1)
        ...
        mpi_dense_lu_test_matrix_rhs(n_shared, distributed_block_rows)
        if n_shared == nproc
            mpi_dense_lu_test_matrix_rhs_no_distributed(n_shared)
        end
    end
end
